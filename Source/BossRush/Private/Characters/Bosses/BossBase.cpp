...
void ABossBase::ExecuteNextPattern()
{
	...
	TArray<TSubclassOf<UGA_PatternBase>>* SelectedPatternArray = nullptr;

	if (Distance < 300.0f)
	{
		SelectedPatternArray = &NearPatterns;
	}
	else if (Distance < 1000.0f)
	{
		SelectedPatternArray = &MiddlePatterns;
	}
	else
	{
		SelectedPatternArray = &FarPatterns;
	}

	if (SelectedPatternArray && SelectedPatternArray->Num() > 0)
	{
		int32 RandomIdx = FMath::RandRange(0, SelectedPatternArray->Num() - 1);
		TSubclassOf<UGA_PatternBase> ChosenAbility = (*SelectedPatternArray)[RandomIdx];

		if (GetAbilitySystemComponent()->TryActivateAbilityByClass(ChosenAbility))
		{
			bIsExecutingPattern = true; // 패턴 실행 시작
			PatternCounter++;
			
			if (PatternCounter >= PatternThresholdForSwitch)
			{
				SwitchTarget();
			}
		}
...
void ABossBase::SwitchTarget()
{
	...
	// 보스 자신과 플레이어가 아닌 캐릭터 제외
	TArray<ACharacterBase*> ValidPlayers;
	for (AActor* Actor : FoundPlayers)
	{
		ACharacterBase* Char = Cast<ACharacterBase>(Actor);
		if (Char && Char != this && Char->Identity == EIdentity::Player)
		{
			ValidPlayers.Add(Char);
		}
	}
